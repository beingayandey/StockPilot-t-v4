const APTableBody = ({ data, selectedRows, toggleRow, deleteSingle, setQuantity, addQuantity }) => {
    return (
        <tbody className="divide-y divide-gray-200">
            {data.map((row) => (
                <tr key={row.id} className={`transition duration-200 hover:bg-light-variant-one  ${data.indexOf(row) % 2 === 0 ? "bg-primary-white" : "bg-light-variant-twelve"}`}>
                    ...
                    <td className="px-2 py-1.5 text-sm font-medium text-primary-black">${row.price.toFixed(2)}</td>

                    {/* ===== Quantity (Stock) editable cell ===== */}
                    <td className="px-2 py-1.5">
